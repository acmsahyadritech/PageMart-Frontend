...
import { getMyPurchaseList} from "./helper";
...
const CheckOrders = () => {
  ...
  return (
    <Base title="Welcome Customer!" description="Manage products here" className="container bg-info p-4">
      ...
      <div className="card text-white box2 mb-3 " style={{maxWidth: "100%"}}>
      {orders.reverse().map((order, index) => {
            return (
              
              <div key={index} className="mb-3 ">
              <div className="card-header text-dark bg-warning border-dark" ><h4><b>Order Id: {order.transaction_id}</b></h4></div>
              <div className="card-body" style={{backgroundColor:"#0f4c75"}}>
              <h4 className="text-white">Shipping Address :- {order.user.address}</h4>
